...
print_help() {
    ...
    echo "  --catalog-direction    Run generation using a catalog direction file"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --catalog-direction) catalog_direction=true; custom_direction=true; which_catalog_direction="$2"; shift 2 ;;
...
if [ "$custom_direction" = false ] && [ "$custom_energy" = false ]; then
    ...
    if [ "$catalog_direction" = true ]; then
        GEN_FCL_CHANGED="${GEN_FCL}_customDirection_cat${which_catalog_direction}"
    else
...
if [ "$SIMULATION_CATEGORY" = "directions" ]; then
    if [ "$catalog_direction" = true ]; then
        echo "Copying fcl files having direction $which_catalog_direction..."
        # move the generated fcl to the standard name
        cat_file=${HOME_DIR}/create_directions/output/${GEN_FCL}_customDirection_$which_catalog_direction.fcl
        if [ ! -f "$cat_file" ]; then
            echo "Catalog direction file $cat_file not found. Exiting..."
            exit 1
        else
            echo "Catalog direction file $cat_file found."
        fi
        cp $cat_file ${DATA_PATH}/${GEN_FCL_CHANGED}.fcl
        echo " "
    else
