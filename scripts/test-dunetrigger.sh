...
if [ $pdune = true ]; then
    for reco_fcl in ${protodune_reco_fcls[@]}; do
        command_protodune=". ${HOME_DIR}/scripts/triggersim.sh -m ${protodune_gen_fcl} -g ${protodune_g4_fcl} -d ${protodune_detsim_fcl} -r ${reco_fcl} -j trig.json -f testPdune"
...
if [ $pdune_data = true ]; then
    ...
    for reco_fcl in ${protodune_data_reco_fcls[@]}; do
        command_protodune=". ${HOME_DIR}/scripts/protoduneana.sh -c ${pdune_decoder} -r ${reco_fcl} -j trig.json -f testPduneData"
        echo " Running command: ${command_protodune}"
        ${command_protodune} 
        if [ $? -ne 0 ]; then
            echo " PROBLEM!!!! While running ${command_protodune}"
...
if [ $onetwosix = true ]; then
    for reco_fcl in ${onetwosix_reco_fcls[@]}; do
        command_1x2x6=". ${HOME_DIR}/scripts/triggersim.sh -m ${onetwosix_gen_fcl} -g $onetwosix_g4_fcl -d $onetwosix_detsim_fcl -r ${reco_fcl} -j trig.json -f test1x2x6"
        if [ $only_reco = true ]; then
            command_1x2x6=". ${HOME_DIR}/scripts/triggersim.sh -M ${onetwosix_gen_fcl} -G $onetwosix_g4_fcl -D $onetwosix_detsim_fcl -r ${reco_fcl} -j trig.json -f test1x2x6"
        fi
...
if [ $onetwotwo = true ]; then
    for reco_fcl in ${onetwotwo_reco_fcls[@]}; do
        command_1x2x2=". ${HOME_DIR}/scripts/triggersim.sh -m ${onetwotwo_gen_fcl} -g $onetwotwo_g4_fcl -d $onetwotwo_detsim_fcl -r ${reco_fcl} -j trig.json -f test1x2x2"
        if [ $only_reco = true ]; then
            command_1x2x2=". ${HOME_DIR}/scripts/triggersim.sh -M ${onetwotwo_gen_fcl} -G $onetwotwo_g4_fcl -D $onetwotwo_detsim_fcl -r ${reco_fcl} -j trig.json -f test1x2x2"
        fi
